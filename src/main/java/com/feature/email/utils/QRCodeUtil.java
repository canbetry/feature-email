...
public class QRCodeUtil {
    ...
    public static String encode(String content, String imgPath, String destPath,
        ...
        BufferedImage image = QRCodeUtil.createImage(content, imgPath,
                needCompress);
        String dirs = LocalDate.now().toString();
        mkdirs(destPath, dirs);
        String file = new Random().nextInt(99999999) + ".jpg";
        ImageIO.write(image, FORMAT_NAME, new File(destPath + "/" + dirs + file));
        return "/upload/" + dirs + file;
    }
     ...
     * @param destPath 存放目录
     */
    public static void mkdirs(String destPath, String dirs) {
        File file = new File(destPath);
        //当文件夹不存在时，mkdirs会自动创建多层目录，区别于mkdir．(mkdir如果父目录不存在则会抛出异常)
        if (!file.exists() && !file.isDirectory()) {
            file.mkdirs();
        }
        File files = new File(destPath + "/" + dirs);
        //当文件夹不存在时，mkdirs会自动创建多层目录，区别于mkdir．(mkdir如果父目录不存在则会抛出异常)
